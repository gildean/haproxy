describe('haproxy', function () {
  ...
  describe('#session', function () {
    it('returns an object with all the sessions if no id is provided', function (done) {
      var proxy = new HAProxy({ socket: '/tmp/fixture.sock' });
      proxy.session(function (err, data) {
        expect(err).to.not.be.instanceOf(Error);
        expect(data).to.be.a('object');
        done();
      });
    });
    it('returns an object with stats from a single session if an id is provided', function (done) {
      var proxy = new HAProxy({ socket: '/tmp/fixture.sock' });
      proxy.session('0x7fc52421de00', function (err, data) {
        expect(err).to.not.be.instanceOf(Error);
        expect(data).to.be.a('object');
        done();
      });
